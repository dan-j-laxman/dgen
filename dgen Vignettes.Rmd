...
```{r}
df1 %>% 
  dplyr::mutate(count_l_is_1 = dgen::row_any_count(l1, l2, l3, value = "1")) %>% 
  dplyr::select(l1, l2, l3, count_l_is_1)
```

#### Likert scale variables with *non-ignored* missing data 

* maxmiss is set to 0. If a case has **no** non-missing values, then a count is returned; otherwise, NA. This is the default.
...
*  There are 3 variables, so maxmiss it set to 2. As long as a case has a non-missing value on 3 - 2 = 1 variables, then a count is returned.
...
*  There are 3 variables, so maxmiss is set to 3. If a case is missing on all 3 variables, a count is still computed.
...
* maxmiss cannot be negative or exceed number of variables
```{r}
df1 %>% 
  dplyr::mutate(count_lm_is_1 = dgen::row_any_count(lm1, lm2, lm3, 
                                                    value = "1", 
